...
export default function ProspectionFormModal({ isOpen, agentId, onClose, onSuccess }: ProspectionFormModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      revenuEstime: parseFloat(formData.revenuEstime) || undefined,
      revenuJournalier: parseFloat(formData.revenuJournalier) || undefined,
      chiffreAffairesMensuel: parseFloat(formData.chiffreAffairesMensuel) || undefined,
    ...
    return (
      <div className="flex flex-col items-center gap-1 min-w-0">
        <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${
          ...
          active ? 'bg-violet-600 text-white ring-2 ring-violet-400/50 shadow-lg shadow-violet-600/30 scale-110' :
  ...
  const inputCls = (field?: keyof FormData) =>
    `w-full px-3 py-2.5 bg-slate-800 border rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-violet-500 transition-all ${
      field && errors[field] ? 'border-red-500/60' : 'border-slate-700'
    }`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4">
      ...
      <div className="relative w-full sm:max-w-md bg-slate-950 border-t sm:border border-slate-800 sm:rounded-2xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-200 flex flex-col max-h-[95vh] sm:max-h-[90vh]">

        {/* ─── HEADER ─── */}
        <div className="bg-slate-900/80 border-b border-slate-800 px-4 pt-4 pb-3 flex-shrink-0">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-500 mt-0.5">Étape {step}/{TOTAL_STEPS}</p>
              </div>
            </div>
            <button onClick={onClose} className="w-8 h-8 rounded-full hover:bg-slate-800 flex items-center justify-center transition-colors border border-transparent hover:border-slate-700">
            ...
            {STEP_META.map((sm, i) => (
              <React.Fragment key={sm.num}>
                ...
                {i < STEP_META.length - 1 && (
                  <div className="h-0.5 flex-1 bg-slate-800 rounded-full overflow-hidden mx-1">
                    <div className={`h-full bg-violet-500 transition-all duration-500 ease-out ${step > sm.num ? 'w-full' : 'w-0'}`} />
                  </div>
                )}
              </React.Fragment>
            ))}
        ...
        {/* Offline */}
        {!navigator.onLine && (
          <div className="mx-4 mt-3 px-3 py-2 bg-amber-500/10 border border-amber-500/20 rounded-lg flex items-center gap-2 flex-shrink-0">
            <WifiOff size={14} className="text-amber-400" />
            <span className="text-[10px] font-medium text-amber-400">Mode hors-ligne</span>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-4 min-h-0">
          ...
          {step === 2 && (
            <div className="space-y-4 animate-in slide-in-from-right-4 fade-in duration-300">
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Activité principale</label>
            ...
            <div className="space-y-4 animate-in slide-in-from-right-4 fade-in duration-300">
              ...
              <div className="space-y-2 bg-slate-900/50 p-3 rounded-xl border border-slate-800">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Revenus</span>
                  <div className="flex bg-slate-800 p-0.5 rounded-lg border border-slate-700">
                    {['Mensuel', 'Journalier'].map(t => (
                      <button key={t} type="button" onClick={() => set('typeRevenu', t)}
                        className={`px-3 py-1 rounded-md text-[10px] font-bold transition-all ${
                          formData.typeRevenu === t ? 'bg-violet-600 text-white shadow' : 'text-slate-500 hover:text-white'
                        }`}>{t}</button>
                    ))}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {formData.typeRevenu === 'Journalier' ? (
                    <div className="space-y-1">
                      <label className="text-[10px] text-slate-500 font-medium">Revenu/jour (FCFA)</label>
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] text-slate-500 font-medium">Revenu/mois (FCFA)</label>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-medium">CA mensuel (FCFA)</label>
                ...
                {formData.typeRevenu === 'Journalier' && formData.revenuJournalier && (
                  <div className="bg-violet-500/10 px-3 py-2 rounded-lg border border-violet-500/20 text-[10px] text-violet-300 flex items-center gap-1.5">
                    <TrendingUp size={10} />
              ...
              {/* Notes */}
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Commentaires agent</label>
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Observations</label>
        ...
        {/* ─── FOOTER ─── */}
        <div className="border-t border-slate-800 p-4 flex gap-3 flex-shrink-0 bg-slate-950/80">
          {step > 1 ? (
            <button type="button" onClick={handleBack}
              className="px-4 py-3 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-xl text-white text-xs font-bold flex items-center gap-1.5 transition-colors">
              <ArrowLeft size={14} /> Retour
            </button>
          ) : (
            <button type="button" onClick={onClose}
              className="px-4 py-3 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-xl text-white text-xs font-bold transition-colors">
              Annuler
            </button>
          )}
          ...
          {step < TOTAL_STEPS ? (
            <button type="button" onClick={handleNext}
              className="flex-1 py-3 bg-violet-600 hover:bg-violet-500 active:bg-violet-700 rounded-xl text-white font-bold text-sm shadow-lg shadow-violet-900/20 transition-all flex items-center justify-center gap-2">
              Suivant <ArrowRight size={14} />
            </button>
          ) : (
            <button type="button" onClick={handleSubmit} disabled={loading}
              className="flex-1 py-3 bg-violet-600 hover:bg-violet-500 active:bg-violet-700 rounded-xl text-white font-bold text-sm shadow-lg shadow-violet-900/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
