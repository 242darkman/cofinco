...
const Logo: React.FC<{ src?: string; alt: string; className?: string }> = ({ src, alt, className }) => {
  ...
  if (!src || err) {
    ...
    return (
      <div className={`${className} bg-slate-900 rounded flex items-center justify-center text-white font-bold text-xl`}>
        {initials}
      </div>
    );
  }
  return (
    <>
      {loading && <div className={`${className} bg-slate-200 rounded animate-pulse`} />}
      <img
        src={src}
        alt={alt}
...
export const InvoiceTemplate = React.forwardRef<HTMLDivElement, InvoiceTemplateProps>(
  ({ data, companyInfo = COMPANY, showQRCode = true }, ref) => {
    ...
    return (
      <div
        ref={ref}
        data-receipt-root
        className="invoice-a4 bg-white text-slate-900 font-sans text-sm leading-normal"
      ...
      >
        ...
        <div className="flex flex-col justify-between h-full" style={{ padding: '15mm' }}>
          ...
          <div>
            {/* ── HEADER ── */}
            <header className="flex justify-between items-start pb-5 border-b border-slate-900">
              <div className="flex gap-3 items-start">
                <Logo src={LOGO_BASE64} alt={companyInfo.nom} className="w-14 h-14" />
                <div>
                  <h1 className="font-bold text-lg uppercase tracking-wider text-slate-900">
                    {companyInfo.nom}
                  </h1>
              ...
              <div className="text-right">
                ...
                <p className="font-mono text-xs text-slate-600 mt-1">#{reference}</p>
            ...
            <div className="grid grid-cols-2 gap-6 mt-6 mb-6">
              ...
              <div className="bg-slate-50 p-4 rounded border border-slate-100">
                ...
                {data.client?.telephone && (
                  <div className="font-mono text-xs text-slate-600 mt-0.5">{data.client.telephone}</div>
                )}
                {maskedAccount && (
                  <div className="mt-2 text-[10px] text-slate-500">
                    Compte: <span className="font-mono font-medium text-slate-700">{maskedAccount}</span>
                  </div>
                )}
              </div>
              ...
              <div className="bg-slate-50 p-4 rounded border border-slate-100">
                ...
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="font-medium text-slate-700">{data.modePaiement || 'Espèces'}</span>
                </div>
                {data.agent && (
                  <div className="flex justify-between text-xs pt-1 border-t border-slate-200 mt-1">
                    <span className="text-slate-500">Agent</span>
                    <span className="text-slate-700">{data.agent.prenom} {data.agent.nom}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <table className="w-full mb-6">
              <thead>
                <tr className="border-b-2 border-slate-900 text-[10px] uppercase font-bold text-slate-500">
                  ...
                  return (
                    <tr key={idx} className="border-b border-slate-100">
                      <td className="py-3 pr-2 align-top">
                        ...
                        {item.details && (
                          <div className="text-slate-500 text-[10px] mt-0.5">{item.details}</div>
                        )}
                      </td>
                      <td className="py-3 text-center font-mono align-top">{qty}</td>
                      <td className="py-3 text-right font-mono text-slate-600 align-top">
            ...
            <div className="flex justify-end mb-6">
              <div className="w-1/2">
                ...
                {tax > 0 && (
                  <div className="flex justify-between py-2 border-b border-slate-100 text-xs">
                    <span className="text-slate-500">Taxes / TVA</span>
                    <span className="font-mono">{fmt(tax)} {devise}</span>
                  </div>
                )}
                <div className="flex justify-between py-3 border-b-2 border-slate-900 bg-slate-50 px-2 mt-2">
                ...
                {data.montantLettres && (
                  <p className="mt-1 text-[10px] text-slate-500 italic text-right">
                    Arrêté à : <span className="font-medium not-italic text-slate-700">{data.montantLettres}</span>
                  </p>
                )}
              </div>
            </div>
            ...
            {data.notes && (
              <div className="mb-4 p-3 bg-amber-50 border-l-4 border-amber-400 rounded-r">
                <p className="text-[10px] font-bold text-amber-700 uppercase tracking-wider mb-0.5">Note</p>
          ...
          <footer>
            ...
            <div className="grid grid-cols-2 gap-12 mb-6">
              <div>
                <div className="text-[10px] font-bold text-slate-400 uppercase mb-10">
                  Signature Client
                </div>
                <div className="border-t border-dashed border-slate-300 pt-1">
                  <span className="text-[8px] text-slate-400">Date et signature</span>
                </div>
              </div>
              <div>
                <div className="text-[10px] font-bold text-slate-400 uppercase mb-10">
                ...
                <div className="border-t border-dashed border-slate-300 pt-1">
                  <span className="text-[8px] text-slate-400">Cachet et signature</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-end gap-4 border-t border-slate-200 pt-4">
              ...
              <div className="flex-1 text-[8px] text-slate-400 leading-relaxed text-justify">
                <p className="mb-0.5">
