...
import { Plus, Briefcase, Eye, Calendar, Mail, Phone, Upload, FileText, ExternalLink, Clock, MessageSquare, CheckCircle, XCircle, Users, ArrowRight, Loader2, ShieldCheck, UserPlus } from 'lucide-react';
...
interface Candidat {
  ...
  // Hiring approval fields
  approvalStatus?: string; // NOT_STARTED, IN_PROGRESS, APPROVED, REJECTED
...
export default function RecrutementManager({
  ...
  const handleSelectCandidat = async (candidat: Candidat) => {
    ...
    // Fetch approval status if candidate is ACCEPTED
          ...
          <div className="space-y-5">
            ...
            {selectedCandidat.statut === StatutCandidature.ACCEPTED && (
              <div className="border-t border-edge pt-4">
                ...
                {/* Onboarding Button - Show when approved */}
                {selectedCandidat.approvalStatus === 'APPROVED' && (
                  <Button
                    variant="success"
