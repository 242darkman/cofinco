      ...
      <div className="space-y-1">
        {lignes.map((l, i) => (
          <div key={i} className="flex justify-between items-center py-1.5 px-2 hover:bg-slate-700/30 rounded text-xs">
            <div className="flex items-center gap-2">
              <span className="text-cyan-400 font-mono text-[10px] w-6">{l.code}</span>
              <span className="text-slate-300">{l.libelle}</span>
            </div>
            <div className="flex gap-4">
              <span className="font-mono text-white w-20 text-right">{l.montantN.toLocaleString()}</span>
              <span className="font-mono text-slate-500 w-20 text-right hidden md:block">{l.montantN1.toLocaleString()}</span>
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        <div className="text-center py-12">
          <RefreshCw className="animate-spin w-8 h-8 text-orange-500 mx-auto mb-3" />
          <p className="text-slate-400 text-sm">Calcul du TAFIRE...</p>
        </div>
      ) : data ? (
        <>
          {/* Synthèse */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <Card variant="default" padding="sm" className="bg-emerald-500/10 border-emerald-500/30">
              <div className="flex items-center gap-1 mb-1">
                ...
                <span className="text-[10px] text-slate-400">Ressources</span>
              </div>
              <div className="text-lg font-bold text-emerald-400">{totalRessources.toLocaleString()}</div>
            </Card>
            <Card variant="default" padding="sm" className="bg-red-500/10 border-red-500/30">
              <div className="flex items-center gap-1 mb-1">
                <MinusCircle size={12} className="text-red-400" />
                <span className="text-[10px] text-slate-400">Emplois</span>
              </div>
              <div className="text-lg font-bold text-red-400">{totalEmplois.toLocaleString()}</div>
            </Card>
            <Card variant="default" padding="sm" className="bg-blue-500/10 border-blue-500/30">
              <div className="flex items-center gap-1 mb-1">
                <Layers size={12} className="text-blue-400" />
                <span className="text-[10px] text-slate-400">Variation BFR</span>
              </div>
              <div className={`text-lg font-bold ${variationBFR >= 0 ? 'text-blue-400' : 'text-amber-400'}`}>
                {variationBFR >= 0 ? '+' : ''}{variationBFR.toLocaleString()}
              </div>
            </Card>
            <Card variant="default" padding="sm" className={excedentRessources >= 0 ? 'bg-emerald-500/20 border-emerald-500/50' : 'bg-red-500/20 border-red-500/50'}>
              <div className="flex items-center gap-1 mb-1">
                <Activity size={12} className={excedentRessources >= 0 ? 'text-emerald-400' : 'text-red-400'} />
                <span className="text-[10px] text-slate-400">Excédent</span>
              </div>
              <div className={`text-lg font-bold ${excedentRessources >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
          ...
          {/* Légende colonnes */}
          <div className="flex justify-end gap-4 text-[10px] text-slate-500 px-2">
          ...
          {/* Excédent */}
          <Card variant="default" padding="sm" className={excedentRessources >= 0 ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-red-500/10 border-red-500/30'}>
            <div className="flex justify-between items-center">
              <span className="text-sm font-bold text-white">Excédent de Ressources (I - II)</span>
              <span className={`text-lg font-bold ${excedentRessources >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
          ...
          <Card variant="default" padding="sm" className={variationTresorerie >= 0 ? 'bg-emerald-500/20 border-emerald-500/50' : 'bg-red-500/20 border-red-500/50'}>
            <div className="flex justify-between items-center py-2">
              <span className="text-sm font-bold text-white">VARIATION DE TRESORERIE</span>
              <span className={`text-xl font-bold ${variationTresorerie >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            ...
            <div className="border-t border-slate-700 pt-2 mt-2 space-y-1">
              {data.tresorerie.map((t, i) => (
                <div key={i} className="flex justify-between text-xs">
                  <span className="text-slate-400">{t.libelle}</span>
                  <span className="text-white font-mono">{t.montantN.toLocaleString()} FCFA</span>
                </div>
              ))}
            </div>
          </Card>
        ...
        <Card variant="default" padding="lg" className="text-center">
          ...
          <p className="text-slate-400">Aucune donnée TAFIRE disponible</p>
        </Card>
      )}
    </div>
  );
