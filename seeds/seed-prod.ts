...
async function seedNotificationSystem(context: SeedContext, dryRun: boolean): Promise<SeedStepResult[]> {
  ...
  const EMAIL_TEMPLATES_DATA = [
    ...
    },
    {
      ...
      subject: 'Nouvelle demande de conge de {{employeeName}} - COFIN&CO-M',
      ...
      contenuText: 'COFIN&CO-M: {{employeeName}} a soumis une demande de conge ({{leaveType}}) du {{startDate}} au {{endDate}} ({{daysRequested}} jours). A traiter.',
      placeholders: 'employeeName,leaveType,startDate,endDate,daysRequested',
    ...
    },
    {
      ...
      contenuText: 'COFIN&CO-M: {{employeeName}} demande un document ({{documentType}}). Urgence: {{urgence}}. A traiter.',
