...
export function registerFinanceRoutes(app: Express) {
  ...
  /**
   * POST /api/sessions-caisse/ensure-active
   * Endpoint idempotent "get-or-create" pour session caisse.
   *
   * 1. Si l'utilisateur a une session active (OPEN, CLOSING_COUNT, CLOSING_VALIDATION)
   *    → la retourne avec { recovered: true }
   * 2. Si l'utilisateur a une session en attente (REQUESTING_FUNDS, FUNDS_DISPATCHED, FUNDS_RECEIVED)
   *    → la retourne avec { pending: true }
   * 3. Si aucune session et body contient caisseId + mode:
   *    - mode='direct' → ouvre via openDirectWithExistingFunds
   *    - mode='request' → ouvre via requestSessionOpening
   * 4. Si aucune session et pas de params de création → retourne null
   *
   * Résiste à : double-clic, refresh, reconnexion, requêtes dupliquées.
   * NOTE: MUST be defined BEFORE /api/sessions-caisse/:id
   */
  app.post("/api/sessions-caisse/ensure-active", requireAuth, attachAbility, async (req, res) => {
    const user = req.session.user!;

    // 1. Chercher session active existante
    const activeSession = await storage.getActiveSessionForUser(user.id);
    if (activeSession) {
      return res.json({ session: activeSession, recovered: true, pending: false });
    }

    // 2. Chercher session en attente (workflow ouverture coffre)
    const pendingSession = await sessionOpeningService.getPendingSession(user.id);
    if (pendingSession) {
      return res.json({ session: pendingSession, recovered: true, pending: true });
    }

    // 3. Pas de session existante — créer si params fournis
    const data = normalizeKeysDeep(req.body) as any;
    if (!data.caisseId) {
      // Pas de params de création → retourne null (le frontend gère)
      return res.json({ session: null, recovered: false, pending: false });
    }
    ...
    if (!isManager) {
      ...
      const isAssigned = assignments.some((a: any) => a.userId === user.id);
      if (!isAssigned) {
    ...
    if (mode === 'request' && data.montantDemande) {
      // Workflow coffre (demande de fonds)
      ...
      if (!result.success) {
        // Si CAISSE_OCCUPIED ou USER_HAS_SESSION → re-fetch (la session vient d'être créée par un concurrent)
        if (result.errorCode === 'CAISSE_OCCUPIED' || result.errorCode === 'USER_HAS_SESSION') {
          const recoveredSession = await storage.getActiveSessionForUser(user.id)
            || await sessionOpeningService.getPendingSession(user.id);
          if (recoveredSession) {
            return res.json({ session: recoveredSession, recovered: true, pending: true });
          }
        }
        const statusMap: Record<string, number> = { CAISSE_OCCUPIED: 409, USER_HAS_SESSION: 409, INVALID_AMOUNT: 400, DB_ERROR: 500 };
        return res.status(statusMap[result.errorCode || 'DB_ERROR'] || 500).json({
      ...
      return res.status(201).json({ session: result.session, recovered: false, pending: true });
    } else {
      // Ouverture directe
      ...
      if (!result.success) {
        // Si conflit → re-fetch (la session vient d'être créée par un concurrent/double-clic)
        if (result.errorCode === 'CAISSE_OCCUPIED' || result.errorCode === 'USER_HAS_SESSION') {
          const recoveredSession = await storage.getActiveSessionForUser(user.id)
            || await sessionOpeningService.getPendingSession(user.id);
          if (recoveredSession) {
            return res.json({ session: recoveredSession, recovered: true, pending: false });
          }
        }
        const statusMap: Record<string, number> = { CAISSE_OCCUPIED: 409, USER_HAS_SESSION: 409, INVALID_BILLETAGE: 400, DB_ERROR: 500 };
        return res.status(statusMap[result.errorCode || 'DB_ERROR'] || 500).json({
          message: result.error, errorCode: result.errorCode
        });
      }
      ...
      return res.status(201).json({ session: result.session, recovered: false, pending: false });
    }
  });
