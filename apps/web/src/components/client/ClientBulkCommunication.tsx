...
export default function ClientBulkCommunication({ clients, onClose, onComplete }: ClientBulkCommunicationProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-gradient-to-br from-surface-base to-surface border border-edge rounded-xl max-w-3xl w-full shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          <div className="bg-status-info-bg border border-status-info/30 rounded-lg p-4">
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {clients.slice(0, 5).map((client) => (
                <span key={client.id} className="px-3 py-1 bg-surface-elevated/50 rounded-full text-xs text-content-secondary">
                  {client.nom}
                </span>
              ))}
              {clients.length > 5 && (
                <span className="px-3 py-1 bg-surface-elevated/50 rounded-full text-xs text-content-secondary">
          ...
          {!results && (
            <>
              <div className="bg-surface/50 border border-edge rounded-lg p-4">
                <label className="block text-sm font-semibold text-content-secondary mb-3">Méthode d'envoi</label>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    <MessageSquare size={24} className={`mx-auto mb-2 ${method === 'sms' ? 'text-accent' : 'text-content-muted'}`} />
                    <p className={`text-sm font-semibold ${method === 'sms' ? 'text-accent' : 'text-content-secondary'}`}>SMS</p>
                  </button>
                  ...
                  >
                    <Mail size={24} className={`mx-auto mb-2 ${method === 'email' ? 'text-status-success' : 'text-content-muted'}`} />
                    <p className={`text-sm font-semibold ${method === 'email' ? 'text-status-success' : 'text-content-secondary'}`}>Email</p>
                  </button>
                </div>
              </div>

              <div className="bg-surface/50 border border-edge rounded-lg p-4">
                <label className="block text-sm font-semibold text-content-secondary mb-2">Modèles rapides</label>
                  ...
                  {method === 'sms' ? (
                    templates.sms.map((template, idx) => (
                      <button
                        ...
                        className="w-full text-left p-2 bg-surface-elevated/30 hover:bg-surface-elevated/50 rounded text-sm text-content-secondary transition"
                      >
                        {template}
                      </button>
                    ))
                    ...
                    templates.email.map((template, idx) => (
                      <button
                        ...
                        className="w-full text-left p-3 bg-surface-elevated/30 hover:bg-surface-elevated/50 rounded text-sm transition"
                      >
                        <p className="font-semibold text-content-secondary">{template.subject}</p>
                        <p className="text-xs text-content-muted mt-1">{template.body.substring(0, 60)}...</p>
                      </button>
                    ))
                  )}
                </div>
              ...
              {method === 'email' && (
                <div className="bg-surface/50 border border-edge rounded-lg p-4">
                  <label className="block text-sm font-semibold text-content-secondary mb-2">Objet de l'email</label>
                  <input
                    type="text"
                    value={subject}
                    onChange={(e) => setSubject(e.target.value)}
                    className="w-full bg-surface-elevated text-content-primary px-4 py-2 rounded-lg border border-edge-strong focus:outline-none focus:ring-2 focus:ring-status-success"
              ...
              <div className="bg-surface/50 border border-edge rounded-lg p-4">
                <label className="block text-sm font-semibold text-content-secondary mb-2">Message</label>
                <textarea
                  value={message}
                  onChange={(e) => setMessage(e.target.value)}
                  className="w-full bg-surface-elevated text-content-primary px-4 py-3 rounded-lg border border-edge-strong focus:outline-none focus:ring-2 focus:ring-accent min-h-[150px]"
              ...
              <div className="bg-accent/10 border border-accent/30 rounded-lg p-4">
                ...
                <div className="bg-surface-elevated/50 rounded p-3 text-sm text-content-secondary whitespace-pre-wrap">
          ...
          {sending && (
            <div className="bg-surface/50 border border-edge rounded-lg p-6">
              ...
              <p className="text-center text-sm text-content-muted">{progress}% - {Math.round((progress / 100) * clients.length)} / {clients.length}</p>
            </div>
          )}

          {results && (
            <div className="space-y-4">
              <div className="bg-gradient-to-br from-surface to-surface-base border border-edge rounded-lg p-6">
                <h3 className="font-bold text-content-primary mb-4 text-center">Résultats de l'envoi</h3>

                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-status-success-bg border border-status-success/30 rounded-lg p-4 text-center">
                    <CheckCircle className="text-status-success mx-auto mb-2" size={32} />
                    <p className="text-xs text-content-muted">Succès</p>
                    <p className="text-3xl font-bold text-status-success">{results.success}</p>
                  </div>

                  <div className="bg-status-info-bg border border-status-info/30 rounded-lg p-4 text-center">
                    <X className="text-status-info mx-auto mb-2" size={32} />
                    <p className="text-xs text-content-muted">Échecs</p>
                    <p className="text-3xl font-bold text-status-info">{results.failed}</p>
                  </div>
                </div>
              </div>
          ...
          {!sending && !results && (
            canSendCommunications ? (
              ...
              <div className="w-full px-6 py-3 bg-status-warning-bg text-status-warning font-semibold rounded-lg flex items-center justify-center gap-2">
